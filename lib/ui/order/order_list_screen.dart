...
class _OrderListScreenState extends State<OrderListScreen>
    ...
    try {
      ...
      if (_quickFilter != null) {
        ...
        if (_quickFilter == 'today') {
          ...
          results =
              results.where((o) => _safeParseDate(o.date)?.isAfter(weekAgo) ?? false).toList();
        } else if (_quickFilter == 'month') {
          final now2 = DateTime.now();
          results = results
              .where((o) {
                final date = _safeParseDate(o.date);
                return date != null && date.month == now2.month && date.year == now2.year;
              })
              .toList();
        }
      }
  ...
  Future<void> _navigateToForm() async {
    await Navigator.push(context, MaterialPageRoute(builder: (_) => const OrderFormScreen()));
    await _loadOrders();
  }
  ...
  Widget _buildQuickFilters() {
    final filters = {'today': 'Today', 'week': 'This Week', 'month': 'This Month'};
    return Padding(
  ...
  Widget _buildInsightCard() {
    ...
    final pendingCount = _filtered.where((o) => _safeDouble(o.pending) > 0).length;
    final paidCount = total - pendingCount;
    final revenue = _filtered.fold<double>(0.0, (s, o) => s + _safeDouble(o.total));
          ...
          child: Column(
            children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                _insightItem(Icons.shopping_bag, "Total", total.toString(), Colors.blue),
                _insightItem(Icons.pending_actions, "Pending", pendingCount.toString(), Colors.orange),
                _insightItem(Icons.check_circle, "Paid", paidCount.toString(), Colors.green),
                _insightItem(Icons.attach_money, "Revenue", revenue.toStringAsFixed(0), Colors.purple),
              ]),
  ...
  Widget _emptyState() {
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 6),
        const Text("Try clearing filters or creating a new order", style: TextStyle(fontSize: 12)),
      ]),
    );
  }
...
return FadeTransition(
  ...
  builder: (context, child) {
    final currentColors = _getGradientColors(index);
    final nextColors = _getNextGradientColors(index);

    final animatedColors = [
      Color.lerp(currentColors[0], nextColors[0], _animController.value)!,
      Color.lerp(currentColors[1], nextColors[1], _animController.value)!,
    ];

    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment.lerp(Alignment.topLeft, Alignment.bottomRight, _animController.value)!,
end: Alignment.lerp(Alignment.bottomRight, Alignment.topLeft, _animController.value)!,
      ...
      children: [
        ...
        Text(
          isPending
              ...
              : "Paid",
          style: TextStyle(
            color: isPending ? Colors.amberAccent : Colors.lightGreenAccent,
            fontWeight: FontWeight.w600,
          ),
        ),
      ],
